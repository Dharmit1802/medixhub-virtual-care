...
interface ScheduleEvent {
  ...
  title: string;
  time: string;
  type: "appointment" | "meeting" | "break";
  patientName?: string;
}
...
const generateWeekSchedule = (): DaySchedule[] => {
  ...
  return days.map((day, index) => {
    ...
    for (let i = 0; i < eventCount; i++) {
      ...
      const eventTypes = ['appointment', 'meeting', 'break'] as const;
      const type = eventTypes[Math.floor(Math.random() * (i === 0 ? 1 : 3))];
      
      events.push({
        id: `event-${index}-${i}`,
        title: type === 'appointment' ? 'Patient Visit' : 
               type === 'meeting' ? 'Staff Meeting' : 'Lunch Break',
        time: `${hour}:${minute}`,
        type,
        patientName: type === 'appointment' ? 
          ['Emma Watson', 'James Smith', 'Lisa Brown', 'Michael Davis', 'Sarah Johnson'][
              ...
              {day.events.length === 0 ? (
                ...
                day.events.map((event) => (
                  ...
                  >
                    ...
                    {event.patientName && (
                      <p className="text-xs truncate opacity-80">{event.patientName}</p>
                    )}
                  </div>
                ))
              )}
