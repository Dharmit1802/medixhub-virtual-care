...
export function EmergencyStatus({ className }: EmergencyProps) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Random fluctuations
      setWaitTime(prev => Math.max(10, Math.min(120, prev + Math.floor(Math.random() * 11) - 5)));
      setOccupancy(prev => Math.max(30, Math.min(95, prev + Math.floor(Math.random() * 7) - 3)));
      setPatientsPredicted(prev => Math.max(5, Math.min(30, prev + Math.floor(Math.random() * 5) - 2)));
  ...
  return (
    <div className={cn(
      ...
      occupancy >= 80 ? "border-red-300 dark:border-red-900" : 
      occupancy >= 50 ? "border-amber-300 dark:border-amber-900" : 
